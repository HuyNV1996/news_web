...
function newsFilterFunc(allPost) {
    ...
    //console.log("recruit", recruitPost)
    ...
    function filterAndPaginate(address, page) {
        var filteredRecruit = filterRecruit(address);
        totalPages = Math.ceil(filteredRecruit.length / pageSize);
        renderPagination(totalPages, page);
        renderPosts(filteredRecruit.slice((page - 1) * pageSize, page * pageSize));
        currentPage = page;
    }

    function filterRecruit(address) {

        return recruitPost.filter(function (post) {
            const arrPost = post.Address;
            return address === 'all' || (arrPost && arrPost.toLowerCase() === address);
        });
    ...
    function renderPosts(jobList) {
        var postsContainer = $('.post-page-recruit');
        postsContainer.empty();
        //console.log("jobList",jobList)
        //console.log("recruitPost", recruitPost)
                            ...
                            <div class="row gap-md-12">
                                ...
                                <div class="col-sm-12 col-md-5 col-lg-5">
                                    <div class="flex align-center gap-4 ">
                                        ...
                                        <div class="one-line">${job.Level.substring(0, 23) + "..."}</div>
                                    </div>
                                </div>
    ...
    function updatePostCount(count, address) {
        var countText = count > 0 ? ` (${count})` : '';
        $('.filter-recruit-btn[data-address="' + address + '"] .post-count').text(countText);
    }

    function updateAllPostCounts() {
        $('.filter-recruit-btn').each(function () {
            var address = $(this).data('address');
            var filteredRecruit = filterRecruit(address);
            updatePostCount(filteredRecruit.length, address);
        });
    }

    $('.filter-recruit-btn').on('click', function () {
        $('.filter-recruit-btn').removeClass('active');
        $(this).addClass('active');

        var address = $(this).data('address');
        filterAndPaginate(address, 1);
    });
    ...
    $('.pagination-container-recruit').on('click', 'a', function (e) {
        ...
        var activeCategory = $('.filter-recruit-btn.active').data('address');
